...
def prediction():
    ...
    def get_historical(quote):
        ts = TimeSeries(key='YF43BPYIAPDH2YRT',output_format='pandas')
    ...
    def LIN_REG_ALGO(df):
        ...
        fig = plt2.figure(figsize=(7.2,4.8),dpi=75)
    ...
    def sentanal(tic):
        ...
        # print(df)
        for col in df[['compound']]:
            columnSeriesObj = df[col]
            for cv in columnSeriesObj.values:
                if cv>0:
                    pos=pos+1
                elif cv<0:
                    neg=neg+1
        ...
        global_polarity=mean_df.mean()[tic]
    ...
    try:
        ...
        # tompricee=stocker.predict.tomorrow(quote)
        ...
        return render_template('index.html',quote=quote,arima_pred=round(arima_pred,2),lstm_pred=round(lstm_pred,2),
